...
async function runLeadFlow(page, context, label, leadId) {
  ...
  const result = {
    ...
    screenshot: path.join(OUT_DIR, `ENG-WB-CSV-001-browser-flow-tester-5-probe.${label}.png`),
    downloadPath: null,
    finalUrl: null,
  ...
  if (download) {
    result.downloadPath = path.join(OUT_DIR, `ENG-WB-CSV-001-browser-flow-tester-5-probe.${label}.csv`);
...
async function runMissingLeadFlow(page, context) {
  ...
  const result = {
    ...
    leadNotFoundVisible: null,
    sarahMitchellVisible: null,
    bodyTextPrefix: null,
    storage: null,
  };
  ...
  const bodyText = await page.locator('body').innerText().catch(() => '');
  result.leadNotFoundVisible = /Lead not found/i.test(bodyText);
  result.sarahMitchellVisible = /Sarah Mitchell/i.test(bodyText);
  result.bodyTextPrefix = bodyText.slice(0, 1000);
  result.finalUrl = page.url();
  result.storage = await storageSnapshot(page, context);
  await page.screenshot({ path: result.screenshot, fullPage: true });
  result.status = !result.exportButtonVisible && !result.loadingVisibleAfterSettle && result.leadNotFoundVisible && !result.sarahMitchellVisible ? 'PASS' : 'FAIL';
  return result;
}
...
(async () => {
  ...
  const outputPath = path.join(OUT_DIR, 'ENG-WB-CSV-001-browser-flow-tester-5-probe.results.json');
